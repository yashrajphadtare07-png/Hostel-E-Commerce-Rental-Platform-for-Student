    ...
    const unsubscribe = onSnapshot(itemsRef, (snapshot) => {
      const itemsData = snapshot.docs.map(doc => {
        ...
        return {
          ...
          trustLevel: data.trust_level || data.trustLevel || 'bronze',
        };
      }) as FeaturedItem[];
      const sorted = itemsData.sort((a, b) => (b.rating || 0) - (a.rating || 0)).slice(0, 4);
      setItems(sorted);
      setLoading(false);
    }, (error) => {
                ...
                >
                  ...
                  >
                    <div className="relative aspect-[4/5] overflow-hidden">
                        <Image
                          src={item.imageUrl || 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800'}
                          alt={item.title}
                          fill
                      ...
                      <div className="absolute top-4 left-4 z-10">
                        <TrustBadge level={item.trustLevel || 'bronze'} size="sm" />
                      </div>
                      ...
                      <div className="absolute top-4 right-4 z-10 backdrop-blur-md bg-black/40 border border-white/10 px-3 py-1.5 rounded-xl flex items-center gap-1">
                        <span className="text-amber-400 font-bold text-base">₹{item.pricePerDay}</span>
